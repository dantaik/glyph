...
一篇文章交易(v2):       gas ≈ 21,000
                            ...
                            + ~2,270                        ← LOG3：1 indexed topic + 3 个 32 字节非索引
                            + 5,000                         ← 温 SSTORE（打包槽）
                            + (首篇 +~17,000)               ← 冷槽初始化
```

**正文（一篇 ~1000 中文字，含 2 个标签）**
brotli 后约 1,400–1,600 字节，整篇 publish 交易约 **~95,000 gas**（相比 v1 在 event 里塞 body 省 ~15%）。
当前 ~0.23 gwei、ETH≈$1,690 下:**≈ $0.037/篇,1000 篇 ≈ $37**。
