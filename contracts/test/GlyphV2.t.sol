...
contract GlyphV2Test is Test {
    ...
    function test_plainPostMovesTheHeadAndEmits() public {
        ...
        assertEq(glyph.count(alice), 1);
        assertEq(glyph.latestBlock(alice), 100);
        ...
        vm.prank(alice);
        glyph.publish(TITLE, PAYLOAD);
        assertEq(glyph.count(alice), 2);
        assertEq(glyph.latestBlock(alice), 150);

        assertEq(glyph.count(bob), 0);
        assertEq(glyph.latestBlock(bob), 0);
    }
    ...
    function test_everyAuthorIsTheirOwnStream() public {
        ...
        vm.prank(alice);
        glyph.publish(TITLE, PAYLOAD);
        assertEq(glyph.latestBlock(bob), 110);
    ...
    function test_thePlainFormTakesNoValue() public {
        ...
        vm.prank(alice);
        (bool ok,) = address(glyph).call{value: 1}(abi.encodeWithSignature("publish(bytes32,bytes)", TITLE, PAYLOAD));
        assertFalse(ok);
    ...
    function test_aZeroHookIsAPlainPost() public {
        ...
        emit Post(alice, address(0), 0, 0, TITLE);
        vm.prank(alice);
        glyph.publish(TITLE, PAYLOAD, address(0), hex"");
        assertEq(glyph.count(alice), 1);
    }
    ...
    function test_valueWithNoHookToForwardItToReverts() public {
        ...
        glyph.publish{value: 1}(TITLE, PAYLOAD, address(0), hex"");
        assertEq(glyph.count(alice), 0);
    ...
    function test_theHookIsHandedEverythingAfterTheEffects() public {
        vm.prank(alice);
        glyph.publish(TITLE, PAYLOAD, address(hook), hex"c0ffee");
        ...
        vm.roll(130);
        vm.prank(alice);
        glyph.publish(TITLE, PAYLOAD, address(hook), hex"");
        c = hook.last();
        assertEq(c.index, 1);
    ...
    function test_theHookIsAnIndexedTopicOfTheEvent() public {
        ...
        assertEq(logs[0].emitter, address(glyph));
    ...
    function test_theEventIsEmittedBeforeTheHookRuns() public {
        ...
        vm.recordLogs();
        ReenteringHook re = new ReenteringHook(glyph);
        vm.prank(alice);
        glyph.publish(TITLE, PAYLOAD, address(re), hex"");
        Vm.Log[] memory logs = vm.getRecordedLogs();
    ...
    function test_valueIsForwardedToTheHookAndNoneIsKept() public {
        vm.deal(alice, 1 ether);
        vm.prank(alice);
        glyph.publish{value: 0.25 ether}(TITLE, PAYLOAD, address(hook), hex"");
    ...
    function test_aFeeHookGatesOnWhatItIsPaid() public {
        ...
        glyph.publish{value: 0.001 ether}(TITLE, PAYLOAD, address(fee), hex"");
        assertEq(glyph.count(alice), 0);

        vm.prank(alice);
        glyph.publish{value: 0.01 ether}(TITLE, PAYLOAD, address(fee), hex"");
    ...
    function test_aHookThatRevertsRevertsThePostWithItsOwnReason() public {
        ...
        glyph.publish(TITLE, PAYLOAD, address(hook), hex"");
        assertEq(glyph.count(alice), 0);
        assertEq(glyph.latestBlock(alice), 0);
    }
    ...
    function test_aHookThatRejectsValueRevertsThePost() public {
        ...
        glyph.publish{value: 1}(TITLE, PAYLOAD, address(hook), hex"");
    ...
    function test_aHookReturningTheWrongSelectorIsRejected() public {
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(GlyphV2.HookRejected.selector, address(hook), bytes4(0xdeadbeef)));
        glyph.publish(TITLE, PAYLOAD, address(hook), hex"");
        assertEq(glyph.count(alice), 0);
    }
    ...
    function test_anAddressWithoutCodeIsNotAHook() public {
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(GlyphV2.NotAHook.selector, bob));
        glyph.publish(TITLE, PAYLOAD, bob, hex"");
    }
    ...
    function test_aContractWithoutOnPublishIsRejected() public {
        ...
        vm.prank(alice);
        vm.expectRevert();
        glyph.publish(TITLE, PAYLOAD, address(glyph), hex"");
        assertEq(glyph.count(alice), 0);
    }
    ...
    function test_aReenteringHookPublishesAsItselfNotAsTheAuthor() public {
        ...
        glyph.publish(TITLE, PAYLOAD, address(re), hex"");
        assertEq(glyph.count(alice), 1);
        assertEq(glyph.count(address(re)), 1);
        assertEq(glyph.latestBlock(address(re)), 100);
    }

    function test_theSameHookServesManyAuthors() public {
        vm.prank(alice);
        glyph.publish(TITLE, PAYLOAD, address(hook), hex"01");
        vm.prank(bob);
        glyph.publish(TITLE, PAYLOAD, address(hook), hex"02");
        assertEq(hook.callCount(), 2);
        RecordingHook.Call memory c = hook.last();
    ...
    function testFuzz_everyPostLinksToTheAuthorsPreviousBlock(uint8 posts, uint16 gap, bool hooked) public {
        ...
        for (uint256 i; i < n; ++i) {
            ...
            vm.prank(alice);
            if (hooked) glyph.publish(TITLE, PAYLOAD, h, hex"");
            else glyph.publish(TITLE, PAYLOAD);
            assertEq(glyph.count(alice), i + 1);
            assertEq(glyph.latestBlock(alice), at);
            prev = at;
            at += step;
        }
    ...
    {
        vm.prank(alice);
        glyph.publish(title, payload, address(hook), hookData);
        RecordingHook.Call memory c = hook.last();
        assertEq(c.payload, payload);
