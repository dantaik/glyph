...
test.describe('the home feed', () => {
  ...
  test('the footer names the contract once and every chain, with what was scanned', async ({ page }) => {
    await prepare(page);
    await page.goto('/');
    await expect(chip(page, '以太坊').first()).toBeVisible();
    const lines = page.locator('footer [data-chain-line]');
    await expect(lines).toHaveCount(2);
    await expect(lines.nth(0)).toContainText('以太坊');
    await expect(lines.nth(1)).toContainText('Taiko');
    await expect(lines.nth(0)).toContainText(/扫描范围 [\d,]+ 至 [\d,]+/);
    await expect(page.locator('footer')).toContainText('合约：0x0000…361C');
  });
  ...
  test('/ethereum opened directly reads Ethereum alone', async ({ page, request }) => {
    ...
    await expect(page.getByText('只看以太坊')).toBeVisible();
