...
/**
 * Live scan status — the scanning label plus, once progress events arrive,
 * the block range being read (or the posts found so far for author-chain
 * walks) and a thin accent bar. Shared by the initial feed scan and both
 * 加载更早的文章 paths.
 */
export default function ScanProgress({ label = '正在扫描…', progress, className = '' }) {
  const pct = progress ? Math.round(progress.fraction * 100) : 0;
  const sameBlock =
    progress && String(progress.fromBlock) === String(progress.toBlock);
  return (
