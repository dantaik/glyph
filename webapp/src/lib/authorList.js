// authorList.js — one author's title list on one chain: a walk that outlives
// the page showing it.
//
// The contract keeps a reverse block-linked list per author, so a list is
// read block by block from `latestBlock(author)` down. An
// AuthorListController owns that walk the way feed.js owns the feed's
// sweeps: it runs to completion whether or not the author page is still
// mounted, hands over each block's rows as they arrive, and persists after
// every fetched block. Coming back to the author later in the session shows
// everything the walk found. There is one controller per (author, chain).
//
// Its rows are the walked chain itself — contiguous from the author's
...
export class AuthorListController {
  ...
  getSnapshot = () => {
    if (this.#snapshot && this.#snapshotVersion === this.#revision) return this.#snapshot;
    ...
    this.#snapshotVersion = this.#revision;
  ...
  async #refresh() {
    ...
    if (head === 0n) {
      this.#rows = []; // the author has never published on this chain
