...
把图片拖入下方区域或点击上传；点击图片或名称即可复制引用，粘贴到正文。`,
  ...
  'hook.unknown': ({ address }) => `钩子 ${address}`,
  'raw.call': ({ call }) => `调用 ${call}`,
  'raw.hook': ({ hook }) => `钩子 ${hook}`,
  'raw.hookData': ({ bytes }) => `钩子数据 ${bytes}`,
  'raw.hookDataHex': '钩子数据',
  'raw.relayed': ({ author, sender }) => `由 ${author} 签名，由 ${sender} 发送`,
  'raw.deadline': ({ when }) => `有效期至 ${when}`,
  'raw.signature': '签名',
  ...
  'hooks.multi': '多个钩子',
  'hooks.address': '钩子地址',
  'hooks.data': '钩子数据（十六进制）',
  'hooks.value': '随同发送的 ETH',
  'hooks.entry': ({ position }) => `钩子 ${position}`,
  'hooks.addHook': '添加钩子',
  'hooks.removeHook': ({ position }) => `移除钩子 ${position}`,
  'hooks.multiNote': ({ address }) =>
    `这些钩子由 ${address} 处的分发器按此顺序依次调用，各自带上自己的数据和分得的 ETH。`,
  'hooks.invalidAddress': ({ position }) => `钩子 ${position}：这不是一个合约地址。`,
  'hooks.invalidHex': ({ position }) => `钩子 ${position}：数据必须是 0x 开头的十六进制。`,
  'hooks.invalidValue': ({ position }) => `钩子 ${position}：这不是一个 ETH 数量。`,
  'hooks.noHooks': '至少添加一个钩子，或选择“无”。',
  'hooks.known': ({ name }) => `本应用称之为${name}。`,
  'cost.hookValue': '随同发送给钩子',
  'cost.hookNote': '钩子本身的开销不在此估算之内。',

  // --- 签名交由他人代发，以及代发别人签好的文章 ----------------------------
  'relay.signButton': '改为签名交由他人代发',
  ...
  'relay.copy': '复制',
  'relay.copied': '已复制',
  'relay.download': '下载 .json',
  'relay.heading': '代发一篇已签名的文章',
  'relay.note':
    '粘贴别人交给你的已签名文章，或打开它的文件。你支付 gas；文章记在签名作者的名下，其中的任何内容都无法更改。',
  'relay.paste': '把已签名的文章粘贴到这里',
  ...
  'relay.expired': '这篇已签名的文章已过期；需要作者重新签名。',
  'relay.wrongChain': ({ chain }) => `这篇已签名的文章是发往${chain}的；请把发布目标切换到该链再发送。`,
  'relay.notJson': '不是有效的 JSON 文件。',
  ...
  'relay.send': '发送到链上',
  'relay.sending': '请在钱包中确认…',
  'relay.sent': ({ chain }) => `已代发到${chain}`,
  'relay.failed': '代发失败',
};
