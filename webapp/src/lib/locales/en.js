...
Drop images into the area below or click to upload; click an image or its name to copy its reference and paste it into the body.`,
  ...
  'hook.unknown': ({ address }) => `hook ${address}`,
  'raw.call': ({ call }) => `Call ${call}`,
  'raw.hook': ({ hook }) => `hook ${hook}`,
  'raw.hookData': ({ bytes }) => `${bytes} of hook data`,
  'raw.hookDataHex': 'Hook data',
  'raw.relayed': ({ author, sender }) => `Signed by ${author}, sent by ${sender}`,
  'raw.deadline': ({ when }) => `valid until ${when}`,
  'raw.signature': 'Signature',

  // --- Choosing a hook in the write tab -------------------------------------
  'hooks.heading': 'Hook',
  'hooks.note':
  ...
  'hooks.multi': 'Several hooks',
  'hooks.address': 'Hook address',
  'hooks.data': 'Hook data (hex)',
  'hooks.value': 'ETH to send along',
  'hooks.entry': ({ position }) => `Hook ${position}`,
  'hooks.addHook': 'Add a hook',
  'hooks.removeHook': ({ position }) => `Remove hook ${position}`,
  'hooks.multiNote': ({ address }) =>
    `The hooks are called in this order by the fan-out at ${address}, each with its own data and its share of the ETH.`,
  'hooks.invalidAddress': ({ position }) => `Hook ${position}: that is not a contract address.`,
  'hooks.invalidHex': ({ position }) => `Hook ${position}: the data must be hex, 0x-prefixed.`,
  'hooks.invalidValue': ({ position }) => `Hook ${position}: that is not an amount of ETH.`,
  'hooks.noHooks': 'Add at least one hook, or choose “None”.',
  'hooks.known': ({ name }) => `Known here as ${name}.`,
  'cost.hookValue': 'Sent along to the hook',
  'cost.hookNote': 'What the hook itself does is not in this estimate.',

  // --- Signing a post for a relayer, and relaying one --------------------------
  'relay.signButton': 'Sign for a relayer instead',
  ...
  'relay.copy': 'Copy',
  'relay.copied': 'Copied',
  'relay.download': 'Download .json',
  'relay.heading': 'Relay a signed post',
  'relay.note':
    'Paste a signed post somebody handed you, or open its file. You pay the gas; the post is recorded under the author who signed it, and nothing in it can be changed.',
  'relay.paste': 'Paste the signed post here',
  ...
  'relay.expired': 'This signed post has expired; the author has to sign it again.',
  'relay.wrongChain': ({ chain }) => `This signed post is for ${chain}; pick that chain as the publish target to send it.`,
  'relay.notJson': 'Not a valid JSON file.',
  ...
  'relay.sending': 'Confirm in your wallet…',
  'relay.sent': ({ chain }) => `Relayed to ${chain}`,
  'relay.failed': 'Could not relay',
};
